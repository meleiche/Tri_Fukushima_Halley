...
void Node_fi::write(char ff[200])
{
	...
	fn << " ERROR"  << endl;
	fn << "---------------------------------------" << endl;
	fn << "err_0_fi(ss*1e6) = " << (err_0_fi * 3600 * 1e6)  << "   err_0_h(nm) = " << (err_0_h * 1e9) << endl;
	fn << "err_1_fi(ss*1e6) = " << (err_1_fi * 3600 * 1e6)  << "   err_1_h(nm) = " << (err_1_h * 1e9) << endl;
	fn << "err_2_fi(ss*1e6) = " << (err_2_fi * 3600 * 1e6)  << "   err_2_h(nm) = " << (err_2_h * 1e9) << endl;	
	//fn << endl ;
	fn.close();
	return;
};
void Node_fi::write_results(char ff[200])
{
	...
	fn << std::fixed << std::setprecision(18) ;
	fn << ID <<","  ;
	fn << (err_0_fi * 3600 * 1e6)  << "," << (err_0_h * 1e9) <<","  ;
	fn << (err_1_fi * 3600 * 1e6)  << "," << (err_1_h * 1e9) <<","  ;  
	fn << (err_2_fi * 3600 * 1e6)  << "," << (err_2_h * 1e9)        ; 
	fn << endl	;
	fn.close();
	return;
} ;