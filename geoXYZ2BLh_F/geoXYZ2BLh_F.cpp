...
Node_Halley *Node_Array_Halley;	  // Array of Node_Halley
...
// ----------------------------------------------------------------------------------------------------------
//	HEader for Halley's Method (OUTPUT_FILE)
// ----------------------------------------------------------------------------------------------------------
void write_Header_Halley_Trace(char ff[20])
{
	ofstream fn;	
	fn.open(ff,ios::out); // new file
...
//	HEader for FULL (OUTPUT_FILE)
// ----------------------------------------------------------------------------------------------------------
void write_Header_OUTFILE(char ff[20])
{
	ofstream fn;  
	fn.open(ff,ios::out); // new file 	
	fn << "********************************************************************\n";
	fn << "              Trilateration Algorithm (Feb 2020)   \n\n";
...
void write_Header_err012(char ff[20])
{
	...
	fn.open(ff,ios::out); // new file
	fn << std::fixed << std::setprecision(18) ;	
	fn << "ID ,err_0_fi(1e6*sec),err_0_h(nm),err_1_fi(1e6*sec),err_1_h(nm),err_2_fi(1e6*sec),err_2_h(nm)"  ;
	fn << endl	;
	fn.close();
	return;	
}
...
// ----------------------------------------------------------------------------------------------------------
//	Header for (Halley_FILE) 
// ---------------------------------------------------------------------------------------------------------
void write_Header_Halley_rslt(char ff[20])
{
	ofstream fn;	
	fn.open(ff,ios::out); // new file
	fn << std::fixed << std::setprecision(18) ;
	fn << "ID,fi_Halley_DD,h_Halley, fi_Halley_err_ss*1e6, h_Halley_err_nm,Delta_Halley_err"  ;	
...
// ----------------------------------------------------------------------------------------------------------
//(fi, lamda, h) to (X,Y,Z)	then (r, Z)
// ----------------------------------------------------------------------------------------------------------
void fi2XYZ_Halley(char out[], char rslt[])
{
	int i = 0 ;	   	
	for(i=0; i<Con.Nodes; i++)
	{
		Node_Array[i].fi2XYZ(WGS84_Datum.a,  WGS84_Datum.f, WGS84_Datum.ee); // geodesy computation				
		Node_Array[i].write_XYZ2(rslt);	
		Node_Array_Halley[i].set_ID(i);
		Node_Array_Halley[i].Zp = Node_Array[i].Zp ;
		Node_Array_Halley[i].rp = sqrt((Node_Array[i].Xp * Node_Array[i].Xp) + (Node_Array[i].Yp * Node_Array[i].Yp) );
		Node_Array_Halley[i].solve_Halley(WGS84_Datum.a, WGS84_Datum.b, WGS84_Datum.ee ) ;
		Node_Array_Halley[i].compute_h_Halley_err(Node_Array[i].h);
		Node_Array_Halley[i].compute_fi_Halley_err(Node_Array[i].fi);
		//Node_Array_Halley[i].compute_Delta_Halley_err(WGS84_Datum.a);
		Node_Array_Halley[i].write_Halley(Halley_FILE);
		Node_Array_Halley[i].write_Halley_Trace(Halley_Trace_FILE);
...
{		
	...
	case WM_COMMAND:  
		...
		if (LOWORD(wParam) == IDOK)
		{
			...
			write_Header_OUTFILE(OUTPUT_FILE) ;
			...
			// Halley Method
			if (IsDlgButtonChecked(hDlg, IDC_RADIO5))		  //IDC_RADIO5	==>> selected  (Halley)
			{
				Node_Array_Halley = new Node_Halley[Con.Nodes];	// create new array of nodes with size [Con.Nodes]
				Fill_Input_Array_fi(Con.Nodes);		    // fill input data inside array
				write_Header_Halley_rslt(Halley_FILE);
				write_Header_Halley_Trace(Halley_Trace_FILE) ;					
				Con.write(out_file);	
				write_Header_XYZ(validate_result);					
				WGS84_Datum.write(out_file);
				fi2XYZ_Halley(out_file, validate_result) ;
			}
			MessageBox(hwnd, "Done", "Done",MB_OK | MB_ICONINFORMATION);
			return (INT_PTR)TRUE;
		}
